...
class ManagedCalendarView {
    ...
    private var selection: CalendarDateSelection {
        didSet {
    ...
    init(theme: Theme,
        ...
        selection = config.initialDateSelection
    ...
    private func dayItemProvider(day: Day) -> AnyCalendarItem {
        ...
        var selectionStyle: CalendarDayViewSelectionStyle = .none

        if let date = day.date(calendar: calendar) {
            status = datesToStatus[date] ?? []
            selectionStyle = selection.styleForDate(date, calendar: calendar)
        } else {
            status = []
        }
    ...
    private func computeSelectedDates(calendar: Calendar) -> [Date] {
        ...
        case .multiDate(let dates):
            _ = dates.map { selectedDates.insert($0) }
        case  .singleRange(_, let range):
            if let range = range {
                _ = calendar.dates(from: range.lowerBound, to: range.upperBound)
